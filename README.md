...
Investigated ~20 GB Supabase egress spike. Root cause: **request frequency, not payload size** — individual API responses are 5-20 KB, but repeated identical queries during development consumed bandwidth. Key findings: (1) added GZip compression middleware reducing all responses by ~60-70%, (2) created consolidated `/api/dashboard/summary` endpoint reducing 5 dashboard API calls to 1 (27 KB → 5.5 KB, 1.6 KB compressed), (3) added 4 composite database indexes for query optimization (+13 MB, 150→163 MB), (4) verified all pagination limits enforced, (5) verified no deliveries dependency in production routes. 33 new Phase 6.1B tests. Full suite: 142 passed (Phase 5.9 + 6.0 + 6.1B), 0 failed. Database: 163 MB (under 500 MB limit). Frontend TypeScript clean, Vite build passes. **Decision: No Redis/CDN needed at current scale — React Query provides adequate browser-side caching.** See `docs/phase-6.1b.md`.
